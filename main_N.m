...
Num_N = 50:20:300; % vary N
Num_iter = 300;% average scene
Num_h_iter = 300;% average channel


% Three Algorithm
GA_Q = zeros(length(Num_N), 1); % proportion of IBFD
GA_capacity = zeros(length(Num_N), 1); % capacity

GCA_Q = zeros(length(Num_N), 1);
GCA_capacity = zeros(length(Num_N), 1);

Greedy_Q = zeros(length(Num_N), 1);
Greedy_capacity = zeros(length(Num_N), 1);

% Random selection of IBFD and OBFD
%Random_Q = zeros(length(Num_N), 1);
%Random_capacity = zeros(length(Num_N), 1);


% lowerbound
%AllI_Q = zeros(length(Num_N), 1); % All IBFD
%AllI_capacity = zeros(length(Num_N), 1);  

AllO_capacity = zeros(length(Num_N), 1); % ALl OBFD 
%AllO_ZF_capacity = zeros(length(Num_N), 1); % All OBFD without modified ZF 
for i = 1:length(Num_N)
    N = Num_N(i); % N antennas
    K = 20; % 20 MUEs
    S = 20; % S SC and SUE
...
% plot the capacity
figure;
plot(Num_N, GA_capacity, '-r*'); % GEA
hold on;
plot(Num_N, GCA_capacity, '-b*'); % DGCA
hold on;
plot(Num_N, Greedy_capacity, '-g*'); % GRA
hold on;
plot(Num_N, AllO_capacity, '-bo'); % All OBFD, lowerbound
grid on;
